    ...
    <div class="container">
      <div class="register">
        ...
        <form @submit.prevent="sub">
          ...
          <div class="password">
            <input id="" v-model="password" type="password" name="" minlength="8" required>
          </div>
          <button>Register</button>
        </form>
        <p>Have an account? <a id="createTxt" href="/login">Login</a></p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async sub () {
      try {
        const usr = auth.createUserWithEmailAndPassword(this.email, this.password)
        console.log(usr)
